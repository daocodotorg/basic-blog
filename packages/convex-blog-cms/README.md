...
Follows [Convex component authoring](https://docs.convex.dev/components/authoring) (the [Components Authoring](https://www.convex.dev/component-authoring) page summarizes directory expectations): `convex.config`, `ComponentApi`, and a `./test` entry for `convex-test`.
...
4. **Open** the UI in your browser:
  **[http://127.0.0.1:3847/admin](http://127.0.0.1:3847/admin)**
  ...
  - Remote / Docker: set **`BLOG_ADMIN_HOST=0.0.0.0`** so the process accepts external connections (the [`Dockerfile.admin`](Dockerfile.admin) used on Railway sets this).
...
Railway sets **`PORT`** automatically; the image sets **`BLOG_ADMIN_HOST=0.0.0.0`**. Health check: **`GET /health`** → `200` + `ok`.
